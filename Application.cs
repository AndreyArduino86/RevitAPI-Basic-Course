...
namespace RevitAPI_Basic_Course
{
    public class Application : IExternalApplication
    {
        public Result OnStartup(UIControlledApplication application)
        {
            ...
            string tabName = "Plagin fo Revit";

            application.CreateRibbonTab(tabName);

            #region 4. Мой первый плагин 
            {
                #region 4.1 Первая программа
                RibbonPanel panel = application.CreateRibbonPanel(tabName, "Первый плагин");

                panel.AddItem(new PushButtonData(nameof(Lab0402_HelloWorld), "Команда1", assemblyLocation, typeof(Lab0402_HelloWorld).FullName)
                {
                    LargeImage = new BitmapImage(new Uri(iconsDirectoryPath + "1.png"))
                });
                #endregion
                #region 4.2 Кнопка для копирования видов уровней

                panel.AddItem(new PushButtonData(nameof(DiologForm), "Команда2", assemblyLocation, typeof(DiologForm).FullName)
                {
                    LargeImage = new BitmapImage(new Uri(iconsDirectoryPath + "2.png"))


                });
                #endregion
                #region 4.3 Кнопка для сортировки видов

                panel.AddItem(new PushButtonData(nameof(SortLevels), "Команда3", assemblyLocation, typeof(SortLevels).FullName)
                {
                    LargeImage = new BitmapImage(new Uri(iconsDirectoryPath + "3.png"))
                ...
                #region 4.4 Кнопка для выбора элементов

                panel.AddItem(new PushButtonData(nameof(LinkSectionBox), "Команда4", assemblyLocation, typeof(LinkSectionBox).FullName)
                {
                    LargeImage = new BitmapImage(new Uri(iconsDirectoryPath + "4.png"))
            ...
            //    panel.AddItem(new PushButtonData(nameof(Lab0502_Transaction), "Вставить смайлик", assemblyLocation, typeof(Lab0502_Transaction).FullName)
            //    {
            ...
            //    panel.AddItem(new PushButtonData(nameof(Lab0503_FilteredElementCollector), "Убрать смайлик", assemblyLocation, typeof(Lab0503_FilteredElementCollector).FullName)
            //    {
